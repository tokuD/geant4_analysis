...
void drawProjection(int run) {
  ...
  std::vector<std::vector<double>> csvData1 = fetchCSV<double>(csvPath0);

  TH1D *projectionX = new TH1D("projectionX", ";x (mm)", 225, -550, 550);
  TH1D *projectionY = new TH1D("projectionX", ";y (mm)", 225, -550, 550);
  ...
  TCanvas *canvasProjectionX =
      new TCanvas("canvasProjectionX", "canvasProjectionX", 800, 800);
  TCanvas *canvasProjectionY =
      new TCanvas("canvasProjectionY", "canvasProjectionY", 800, 800);
  ...
  TString pdfPath =
      Form("../../images/%d/projection/run" + runNum + "_pos0.pdf", cone);
